...
def _build_page_table_v2(manager) -> KVCachePageTable:
    ...
    indexed by layer_group_id while pool_group_idx points at the shared physical
    pool group entry. Pool roles remain native manager strings so custom cache
    buffers do not require a disaggregation-side role registry.
    """
